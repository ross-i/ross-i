...
from utilities.dense_attention import scaled_attention
import sys
import time
...
class hisan(object):
    ...
    class hisan_model(Model):
        ...
        def __init__(self,embedding_matrix,num_classes,attention_size,attention_heads,dropout):
            ...
            self.word_drop = layers.Dropout(self.dropout)
            ...
            self.word_self_att = scaled_attention(use_scale=1/np.sqrt(attention_size),dropout=self.dropout)
            self.word_targ_att = scaled_attention(use_scale=1/np.sqrt(attention_size),dropout=self.dropout)
            
            self.line_drop = layers.Dropout(self.dropout)
            ...
            self.line_target = tf.Variable(tf.random.uniform(shape=[1,self.attention_heads,1,
                               int(self.attention_size/self.attention_heads)]))
            self.line_self_att = scaled_attention(use_scale=1/np.sqrt(attention_size),dropout=self.dropout)
            self.line_targ_att = scaled_attention(use_scale=1/np.sqrt(attention_size),dropout=self.dropout)

            self.doc_drop = layers.Dropout(self.dropout)
    ...
    def __init__(self,embedding_matrix,num_classes,max_sents=201,max_words=15,
                 attention_heads=8,attention_size=400,dropout=0.1):
    
        self.ms = max_sents
        self.mw = max_words
        self.unk_tok = embedding_matrix.shape[0] - 1
        self.vocab_size = embedding_matrix.shape[0]
        self.model = self.hisan_model(embedding_matrix,num_classes,attention_size,attention_heads,dropout)
...
if __name__ == "__main__":
    ...
    model = hisan(vocab,num_classes,max_sents,max_words,
                  attention_heads,attention_size,dropout)
    model.train(X_train,y_train,batch_size,epochs,validation_data=(X_valid,y_valid),
                savebest=True,filepath='savedmodels/hisan.ckpt')
    model.load('savedmodels/hisan.ckpt')

    print("final test accuracy:")
    print("micro: %f, macro: %f" % model.score(X_test, y_test))
